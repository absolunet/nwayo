...
class AbstractWebpackDriver extends Driver {
		...
		await new Promise((resolve, reject) => {
			builder.run((error, stats) => {
				if (error || stats.hasErrors()) {
					...
					this.event.emit(this.events.error, { error: thrownError, stats });
				...
				this.event.emit(this.events.completed, { stats });
	...
	getBuilder(config) {
		...
		builder.compilers.forEach((compiler) => {
			const { taps } = compiler.hooks.invalid;

			const index = taps.findIndex(({ name }) => {
	...
	onStart(handler) {
		return super.onStart(([compiler]) => {
			const { bundle } = compiler.options.plugins.find(({ name }) => {
				return name === 'nwayo';
			}) || {};
	 ...
	 */
	onError(handler) {
		return super.onError(({ error, stats }) => {
			const getStatErrorResource = (s, compilation) => {
				const { stats: substats } = s;

				if (substats) {
					return substats.reduce((total, substat) => {
						return total.concat(getStatErrorResource(substat, compilation || substat.compilation))
							.filter(({ path }, i, array) => {
								return array.slice(i + 1).every(({ path: otherPath }) => {
				...
				return (compilation || s.compilation).errors.map((compilationError) => {
					...
					const { message } = compilationError;
		...
		return (Array.isArray(webpackConfig) ? webpackConfig : [webpackConfig]).map((config) => {
			return {
				...
				plugins: config.plugins.filter((plugin) => {
					return !(plugin instanceof FriendlyErrorWebpackPlugin);
				})
			};
		});
	}
	...
	enableProgress(builder) {
		new this.webpack.ProgressPlugin((percent) => {
			this.event.emit(this.events.progress, { percent });
		}).apply(builder);
