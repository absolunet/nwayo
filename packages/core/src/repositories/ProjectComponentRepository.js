...
class ProjectComponentRepository {
	...
	async all() {
		...
		const files = await this.fs.scandir(componentsPath, 'file', { recursive: true, fullPath: true });

		const components = await Promise.all(files
			.filter((file) => {
				return file.endsWith(`/${this.nwayoConstantPackage.PACKAGE_JSON}`);
			}).map(async (file) => {
				const { name }      = await this.dependency.inFolder(this.pathHelper.dirname(file)).loadPackageJson();
