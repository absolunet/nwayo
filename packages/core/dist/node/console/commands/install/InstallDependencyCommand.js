 ...
 * @hideconstructor
 */
class InstallDependencyCommand extends _Command.default {
  /**
   * @inheritdoc
   */
  get name() {
  ...
  get parameters() {
    return [['dependency', false, null, this.t('commands.install-dependency.parameters.dependency')], ['component', false, null, this.t('commands.install-dependency.parameters.component')]];
  }
  /**
   * @inheritdoc
   */
  ...
  async resolveParameters() {
    const {
      dependency,
      version
    } = await this.resolveDependencyAndVersion();
    const {
      component,
      path
    } = await this.resolveComponentAndPath();
    return {
      dependency,
      version,
      component,
  ...
  async resolveDependencyAndVersion() {
    ...
    const availableVersions = await this.app.make('dependency').inFolder(this.app.make('nwayo.project').getSourcePath()).getAvailableVersions(dependency);
  ...
  async installDependency({
    ...
    await this.app.make('dependency').inFolder(path).save(dependency, version);
    await this.call('install:components');
  }

}

var _default = InstallDependencyCommand;
