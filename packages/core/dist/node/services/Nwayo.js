...
class Nwayo {
  ...
  init() {
    (0, _privateRegistry.default)(this).set('hooks', {});
    (0, _privateRegistry.default)(this).set('calls', new Map());
  }
  /**
  ...
  registerHook(type, phase, action, weight = 0) {
    const hooks = (0, _privateRegistry.default)(this).get('hooks');
    hooks[type] = hooks[type] || {};
    hooks[type][phase] = hooks[type][phase] || [];
    hooks[type][phase].push({
      action,
      weight
    });
    return this;
  ...
  async process(bundles, types, options) {
    ...
    const config = []; // Since each builder driver can be limited in concurrency while generating configuration,
    ...
    for (const index in bundles) {
      if (Object.prototype.hasOwnProperty.call(bundles, index)) {
        ...
        const builder = this.nwayoBuilder.make({
          bundle
        });
  ...
  getHooks(type, phase) {
    ...
    const hooks = (0, _privateRegistry.default)(this).get('hooks')[type];

    if (!phase) {
      return Object.fromEntries(Object.entries(hooks).map(([key, typeHooks]) => {
        return [key, typeHooks.map(({
          action
        }) => {
          return action;
        })];
  ...
  prepareHooks() {
    Object.values((0, _privateRegistry.default)(this).get('hooks')).forEach(hooks => {
  ...
  useState(action, type, phase) {
    const callsMap = (0, _privateRegistry.default)(this).get('calls');
    ...
    callsMap.set(action, { ...phaseCalls,
      [phase]: { ...calls,
        [type]: (calls[type] || 0) + 1
      }
    });
