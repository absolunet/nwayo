...
module.exports = {
	...
	argv: (argv, cwd) => {
		...
		const isFlag = context.command.slice(0, 2) === '--';
		...
		if (context.command !== 'index' && (isFlag || fs.existsSync(`${__dirname}/${context.command}.js`))) {
			...
			if (isFlag) {
				...
				if (context.command === '--tasks') {
					...
					files.forEach((file) => {
						if (file !== 'index.js' && file.slice(0, 5) !== 'flag-') {
							tasks.push(file.slice(0, file.length - 7));
						}
					});
					...
					files.forEach((file) => {
						if (file.slice(-5) === '.yaml') {
							bundles.push(file.slice(0, file.length - 5));
						}
					});
					...
					return require(`../tasks/flag-${context.command.slice(2)}`).run(context);  // eslint-disable-line global-require
				}
