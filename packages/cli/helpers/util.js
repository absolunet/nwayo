...
const obnoxiousMessage = (update = {}) => {
	return `Update available ${chalk.dim(update.current)} ${chalk.reset("→")} ${chalk.green(
		update.latest
...
class Util {
	...
	get usageData() {
		return {
			tasks: {
				...
				grow: [
					`grow ${chalk.reset("[")}${chalk.yellow("extension")}${chalk.reset("]")}`,
					`Generate new project or extension`,
				],
				docs: [`docs`, `Open documentation`], // eslint-disable-line unicorn/prevent-abbreviations
	...
	get usageTasks() {
		...
		let tasks = "";
		const max = Math.max(
			...Object.values(this.usageData.tasks).map(([item]) => {
				return stringWidth(item);
			})
		);
	...
	cmd(command, optionalFlag) {
		...
		if (argv._[0] === action && (argv._.length === 1 || (argv._.length === 2 && argv._[1] === scope))) {
