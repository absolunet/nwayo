...
module.exports = () => {
	...
	flow.createTask("scripts-constants", ({ taskName }) => {
		...
		for (const name of Object.keys(env.bundles)) {
			...
			streams.push(
				toolbox
					.vinylStream(paths.filename.konstanScripts, `var konstan = ${JSON.stringify(data, null, "\t")};`)
					...
					streams.push(
						toolbox
							...
							.on("finish", () => {
								toolbox.log(
									taskName,
									`'${destination}/${filename}' written`,
									toolbox.filesize(`${paths.directory.root}/${destination}/${filename}`)
								);
							})
					);
				}
