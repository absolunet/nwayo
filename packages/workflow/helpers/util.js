...
class Util {
	...
	getGeneratedBanner(name, type, extension) {
		const banner = `Generated by ${env.id} ${env.workflowConfig.version} ${
			extension ? `/ ${extension.id} ${extension.version} ` : ""
	...
	getScriptsUrl(bundle, collection) {
		return `${env.bundles[bundle].output.url}/${paths.build.scripts}${
			collection ? `/${collection}.${paths.extension.scripts}` : ""
		}`;
	}

	getStylesUrl(bundle, collection) {
		return `${env.bundles[bundle].output.url}/${paths.build.styles}${
			collection ? `/${collection}.${paths.extension.stylesBuild}` : ""
		}`;
	}
	...
	checkInstalledWorkflow() {
		...
		if (semver.gt(requiredVersion, installedVersion)) {
			terminal
				.echo(
					boxen(
						`Workflow update available ${chalk.dim(installedVersion)} ${chalk.reset("→")} ${chalk.green(
							requiredVersion
						)}
	...
	initCli(globalUsage) {
		...
		cli.setUsageTasks(
			Object.assign(
				...
				{
					run: [
						`run ${cli.placeholder("<task>")} ${cli.optionalPlaceholder("<bundle>")}`,
						`Run a task ex:[${tasks.join("|")}]`,
						[tasks],
					],
					rebuild: [
						`rebuild ${cli.optionalPlaceholder("<bundle>")} ${cli.optional("--prod")}`,
						`Rebuild the entire project from scratch`,
					],
					watch: [
						`watch ${cli.optionalPlaceholder("<bundle>")}`,
						`Listens for changes on files and run appropriate tasks`,
					],
				},
				globalUsage.tasks
			)
		...
		cli.setFullUsage(Object.assign({}, { Project: ["run", "rebuild", "watch"] }, globalUsage.full), { showBin: false });
	}
}
